...
function buildEditRoutes(selection) {
  ...
  const routes = [
    ...
    },
    {
      ...
      when: "컴포넌트 구조/접근성/SVG 아이콘 변경, 폼 필드 추가, TextField를 화면에 조립",
      open: [domain.frontend.stack, docPath("frontend/ui/component"), ...domain.frontend.ui.filter((path) => path !== docPath("frontend/ui/component"))],
      thenRead: [domain.core.naming, domain.core.codeStyle],
      mustHold: [
        "필드는 NameTextForm처럼 의미 단위로 닫는다. feature/page는 TextField를 직접 쓰지 않는다.",
      ],
    },
    {
      id: "frontend-form-meaning-unit",
      action: "add-component",
      when: "폼 필드/입력/TextField/ChipButton을 feature나 page에서 직접 쓰거나 의미 단위로 닫기",
      open: [docPath("frontend/ui/component")],
      thenRead: [docPath("frontend/ui/screen"), domain.core.naming],
      mustHold: [
        "필드는 NameTextForm처럼 의미 단위로 닫는다. feature/page는 TextField를 직접 쓰지 않는다.",
      ],
    },
    {
      ...
      action: "build-screen",
      when: "새 화면/route/dashboard/settings/list-detail/form workflow/주요 page layout 변경",
