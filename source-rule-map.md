...
- `2.1`, `2.2`, `2.4`, `2.5`, `2.6` -> `core/code-hygiene.md`. `2.5`는 유틸 성격이면 한 곳이어도 분리. `2.6`은 도메인 루트 덤프를 관심사 하위 폴더로 나누고, 논리적 구현 단위 20개 이상을 역할 혼재도와 함께 점검한다.
- `3.x` -> `core/naming.md`.
...
- `1.5`, `1.7`, `1.8`, `5.x` -> `frontend/component.md`. `5.5`는 한 의미의 tsx/css/훅/`type.ts`를 자기 폴더에 둔다.
...
- `testing.md`: goal-driven verification, scope split, and non-colocated test folders that mirror source domains.
