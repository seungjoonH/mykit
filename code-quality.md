...
### 2.6 도메인 폴더 루트에 구현 파일을 쌓지 않는다

도메인 폴더 루트에 무관한 구현 파일을 십수 개 쌓지 않는다. 관심사 하위 폴더로 나눈다.
파일 하나마다 폴더를 만들지 않는다. `utils/`의 `date.ts`처럼 관심사 하나 = 파일 하나인
순수 유틸은 폴더로 감싸지 않는다. `api.ts`처럼 그룹 공용 facade는 도메인 폴더에 남긴다.

과밀도는 물리 파일 수가 아니라 논리적 구현 단위로 판단한다. 같은 이름의 tsx, CSS Module,
훅, 타입 파일은 한 단위다. 한 디렉터리의 직접 구현 단위가 20개 이상이면 구조를 점검하되,
숫자만으로 나누지 않고 역할 혼재도와 응집도를 함께 본다. 같은 추상화 수준의 디자인 시스템은
평면 구조를 허용할 수 있지만 공용 폴더에 도메인 구현이 섞이면 해당 도메인 계층으로 옮긴다.

비코로케이션 테스트 폴더도 이 기준으로 점검한다. 테스트 디렉터리는 가능한 한 소스의 도메인
구조를 반영한다. 검사 결과는 `개선 필요`, `개선 후보`, `허용`으로 분류해 근거와 함께 보고한다.
...
### 5.5 타입 정의 파일 분리와 유닛 폴더

컴포넌트의 타입은 같은 디렉토리의 `type.ts`에 분리한다. tsx, css, 훅, `type.ts`처럼 한
의미의 파일이 여럿이면 자기 폴더에 같이 둔다. 계층·도메인·라우트 루트에 그 파일들을
형제로 쌓지 않는다. `index.ts` barrel은 미리 만들지 않는다. `page.tsx`, `layout.tsx`,
`route.ts`는 프레임워크 자리이므로 덤프가 아니다. 그 옆에 쌓인 폼·훅은 유닛 폴더로 묶는다.
...
- [ ] tsx/css/훅처럼 한 의미의 파일이 계층·라우트 루트에 형제로 쌓여 있는가?
- [ ] 도메인 폴더 루트에 무관한 구현 파일을 쌓지 않고 관심사 하위 폴더로 나눴는가?
- [ ] tsx/CSS Module/훅/type을 논리적 구현 단위로 묶어 과밀도를 판단했는가?
- [ ] 직접 구현 단위가 20개 이상이면 숫자뿐 아니라 역할 혼재도와 응집도를 확인했는가?
- [ ] 비코로케이션 테스트 폴더가 가능한 한 소스의 도메인 구조를 반영하는가?
...
| 날짜 | 내용 |
|---|---|
| 2026-08-29 | 폴더 과밀도를 논리적 구현 단위로 판단하고 20개 이상을 점검 신호로 삼되 응집도와 역할 혼재도를 함께 보도록 확장. 비코로케이션 테스트의 소스 도메인 구조 반영과 감사 결과 등급 추가 |
| 2026-08-17 | 유닛 폴더(5.5)와 도메인 관심사 하위폴더(2.6). 한 의미의 파일은 자기 폴더, 도메인 루트 덤프는 관심사로 나눈다. 파일마다 폴더는 만들지 않는다 |
