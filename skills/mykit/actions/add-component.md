...
사용자가 승인하면 바로 구현한다. 사용자가 수정하면 수정된 기준을 반영한 뒤 구현한다.
...
3. 기존 컴포넌트 계층과 스타일 규칙에 맞춰 구현한다.
4. 불필요한 추상화나 새 dependency를 만들지 않는다.
5. 관련 테스트, typecheck, lint 중 가장 작은 검증을 먼저 실행한다.
