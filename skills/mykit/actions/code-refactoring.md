...
프론트엔드(React 계열) 전용 체크리스트다. 0(SSOT), 1(컴포넌트/훅 경계), 3(컴포넌트 API), 5(사이드
이펙트), 7(성능)은 컴포넌트/훅 존재를 전제한다. 2(데이터 요청 계층)와 6(정리/utils)만 언어나 스택에
무관하게 적용된다. 백엔드 전용 프로젝트에서는 2, 6과 karpathy-guidelines 원칙만 적용하고 나머지는
건너뛴다.
...
대상이 명시되지 않았으면 Project Scan 전에 먼저 하나만 확인한다.

- 전체 코드베이스
- dirty worktree (지금 변경된 파일)
- 사용자 지정 (파일/디렉터리를 직접 알려줌)

`전체 코드베이스`를 고르면 모든 파일을 읽지 않는다. 체크리스트 항목별 키워드로 먼저 후보
파일을 좁히고, 좁혀진 파일만 읽는다.

| 체크리스트 | grep 키워드 |
|---|---|
| 0. SSOT | `<input type="checkbox"`, `<select` |
| 1. 컴포넌트/훅 경계 | 컴포넌트 파일 안의 `fetch(`, 한 파일에 `useState`가 여러 번 |
| 2. 데이터 요청 계층 | `fetch(`, `try {`, `.json()` |
| 5. 사이드 이펙트 | 한 파일에 `useEffect(`가 여러 번 |
| 7. 성능 | `useMemo(`, `useCallback(` |

3(컴포넌트 API 설계), 4(데이터 설계), 6(정리/utils)은 키워드로 좁히기 어렵다. 위에서 좁혀진
후보 파일을 읽을 때 함께 확인한다. 키워드로 좁힌 뒤에도 후보가 너무 많으면(예: 수십 개 이상)
fork나 서브에이전트로 위임한다.
...
- 서로 다른 종류의 로직(폼 상태, validation, 요청 등)이 훅 하나에 몰려 있는가. 로직별로
  별도 훅으로 분리돼야 한다.
...
- 호출부(컴포넌트, 훅)가 URL, method, header를 직접 조립하는가. 도메인 서비스 함수로 은닉돼야 한다.
...
1. 대상이 명시되지 않았으면 대상 확인 질문(전체 코드베이스/dirty worktree/사용자 지정)부터 한다.
2. `code-quality.md`(또는 프로젝트 `playbook/`) 규칙과 디자인 시스템 SSOT 목록을 먼저 확인한다.
