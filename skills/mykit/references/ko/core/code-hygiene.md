...
- 도메인 폴더 루트에 무관한 구현 파일을 쌓지 않는다. 관심사 하위 폴더로 나눈다. 파일마다 폴더를 만들지 않는다.
- 같은 이름의 tsx, CSS Module, 훅, 타입 파일은 한 논리적 구현 단위로 센다. 직접 구현 단위가 20개 이상이면 역할 혼재도와 응집도를 함께 점검한다. 숫자만으로 폴더를 나누지 않는다.
- 같은 추상화 수준의 디자인 시스템은 평면 구조를 허용할 수 있다. 공용 폴더에 도메인 구현이 섞이면 해당 도메인 계층으로 옮긴다.
