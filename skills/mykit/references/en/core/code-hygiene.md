...
## Boundaries
- `utils/` holds pure functions with no domain knowledge.
- If domain rules or API calls creep in, it belongs in a domain service or hook, not `utils/`.

## Test Scope
- Pure functions only need input/output unit tests.
- Cover locale and edge-case inputs (empty array, zero, null).
