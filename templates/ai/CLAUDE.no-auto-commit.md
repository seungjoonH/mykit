...
Start from `playbook/PLAYBOOK.index.yaml` to decide which playbook document to open first.
- Follow `entrypoints` for the default read order.
- Use `domainMap` to jump to stack/domain-specific rules.
- Use `editRoutes` to map requested changes to exact target docs.
...
## 9. Commits — Explicit Approval Only
